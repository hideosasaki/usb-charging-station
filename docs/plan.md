...
|用途|使用デバイス|電源|備考|
|---|---|---|---|
|**(1) 充電ステーション監視(本ドキュメント)**|**Waveshare RP2040-Zero**|SW3518 の 5V ランド|PIO I²C により 3 系統 I²C 構成、デュアルコアで処理分離|
...
LiFePO4 (12.0〜14.4V) → 主ヒューズ → メインヒューズ系統2 (15A) → 充電ステーション (12V 1本) 
│ 
└── SW3518 ×3 (12V 直結、4.5〜24V 対応) 
│ 
└── 5V ランド ──── RP2040-Zero VBUS/5V ピン 
│ 
└── 内蔵レギュレーターで 3.3V 生成 
│ 
├── ILI9341 VCC 
...
- SW3518 の 5V ランド: 内部生成された 5V を MCU 給電に活用、追加部品不要

#### 5V 取得の冗長性確保

充電ステーションの動作を全ポート空き時にも安定させるため、**3 個の SW3518 から SBD(ショットキーバリアダイオード)経由で OR 接続**して 5V を取り出す。

```

SW3518 #1 5V ─[SBD]─┐ SW3518 #2 5V ─[SBD]─┼── RP2040-Zero VBUS SW3518 #3 5V ─[SBD]─┘

```

電圧降下は約 0.3V で 4.7V 程度になるが、RP2040-Zero の VBUS 入力下限を満たす。

#### フォールバック電源(SW3518 5V が不足する場合)

万一 SW3518 の 5V ランドの電流容量・挙動が不足する場合は、**BP5293-50 (秋月 111188、¥240) を別途実装**する。

- 入力 7〜26V、出力 5V 1A、ROHM 製 SIP3
- v5 の 12〜14.4V を完全カバー
- SW3518 5V 系統と並列接続(SBD で分離)
...
|USB-PD 充電 IC|SW3518|3|モジュール形態、5V ランド付き|
|TFT 液晶|ILI9341 2.4" 320×240 + XPT2046 タッチ|1|タッチは未配線|
|ショットキーダイオード|1N5817 等|3|SW3518 5V OR 接続用|
|ユニバーサル基板|サイズ要検討|1||
...
|部品|型番|数量|備考|
|---|---|---|---|
|3.3V/5V DC-DC|BP5293-50|1|SW3518 5V 不足時の代替|
...
│ │ 1h     │ 1h     │        │       │
│ └────────┴────────┴────────┘       │
│ Total: 20.9W   経過 23min          │
└────────────────────────────────────┘
```

**表示項目**:

- 各ポート: 電圧 (V)、電流 (A)、電力 (W)、PD/QC プロトコル種別
- 各ポート: 過去 1 時間の電力推移スパークライン
- 充電フェーズ判定(CC / CV / 完了)
- セッション経過時間
- 全体の合計電力
...
- **データ構造**: 各ポートごとにリングバッファ
- **DataPoint 構造**: timestamp + vout + iout + protocol_id ≒ 10B
- **保持期間**: 直近 1 時間 × 1Hz = 3600 サンプル
- **合計サイズ**: 3 ポート × 3600 × 10B ≒ 約 110 KB

ILI9341 のフレームバッファ(部分更新で運用、フルバッファ不要)、Arduino-Pico ランタイム、各種ライブラリと合わせても 264KB に収まる。

**用途**:

1. 液晶のスパークライン描画(直近 1 時間分)
2. 充電フェーズ判定(直近数分の電流推移を見て CC/CV/完了を推測)
3. セッション統計(開始時刻、積算 Wh)

### 6.3 充電フェーズ判定ロジック

リチウムイオン電池の典型的な充電パターンから推測:
...
**部品発注(進行中)**
- SW3518 モジュール、SBD (1N5817)、その他部品

**SW3518 到着後**
- §9.1 の検証手順で 5V ランド実測(`docs/phase0-measurements.md` に記録予定)
- §9.5 の SBD OR 接続テスト
- 結果に応じて BP5293-50 採否を判断、§7 Phase 1 へ進む
...
### 8.3 5V 系統を SW3518 から取得する根拠

- 追加の DC-DC モジュール不要、配線シンプル
- 12V → 5V 変換が SW3518 内部で既に行われており、その副産物を活用
- フォールバックとして BP5293-50 (¥240) を準備可能
...
### 9.1 SW3518 5V ランドの電流容量

SW3518 内部生成の 5V を MCU 給電に流用する設計だが、各モジュールの 5V ランドの電流容量・挙動はモジュール依存。

**検証方法**: SW3518 モジュール到着後、テスターで 5V 出力を測定。

- 全ポート空き時に 5V が出ているか
- 単独ポートで 200-300mA を引けるか

**フォールバック**: 不足する場合は BP5293-50 を追加実装。
...
SBD で 3 個の SW3518 5V を OR 接続する設計だが、SW3518 の 5V 出力タイミング・レギュレーション特性によっては逆流や電圧変動が発生する可能性。

**検証方法**: Phase 0 でブレッドボード上に SBD OR 回路を組み、SW3518 のポート抜き差し時の 5V ラインの安定性をテスター(可能ならオシロスコープ)で確認。
...
- BP5293-50 データシート(フォールバック用): https://akizukidenshi.com/goodsaffix/BP5293_DataSheet_J_VerC_161013.pdf
...
Phase 0 で 5V ランドの実挙動を確認し、Phase 2 から順次実装。
