...
- repair tick: 3 秒ごとにパネル設定を再送し、完了時に全フィールドを
  再描画する (詳細は 6.1 の blind repair 項)
...
#### パネル自動復旧 (定期 blind repair)

MCU と液晶はデュポン端子で接続しており、液晶を動かすと SPI 信号や
パネル電源が瞬断することがある。パネルは write-only 構成 (MISO / RST
未配線) のため瞬断をファームウェアから検出できず、検出なしの定期修復で
対処する。

- 3 秒周期で冪等な設定コマンドを再送する。SLPOUT → (10 ms 後) MADCTL /
  COLMOD / DISPON。正常なパネルには見た目の変化なし、電源瞬断で素の
  状態に戻ったパネルはこれで復帰する
- 再送完了時に `DisplayUi::refresh()` で静的枠を描き直し、diff キャッ
  シュを無効化 → 次の render で全フィールドが sprite 経由で再描画される
- どちらの瞬断モードでも最大 3 秒 + 1 tick で自動復帰する。ユーザー操作
  は不要

制約: fillScreen を周期実行すると正常時に画面が点滅するため行わない。
そのため電源瞬断後はフィールド矩形の外側 (行間の余白など) にノイズが
残り得る。実害が出る場合の拡張余地は MISO を配線した読み返し検出
(RDMADCTL 比較) による即時フル再初期化。
