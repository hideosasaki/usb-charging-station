...
void test_attach_starts_session(void) {
  SessionStats s{};
  session_update(s, mk(5000, 300, true, 1000), Rail::UsbC, 1000);
...
void test_detach_stops_session_but_keeps_energy(void) {
  SessionStats s{};
  session_update(s, mk(9000, 2000, true, 1000), Rail::UsbC, 1000);  // start
  session_update(s, mk(9000, 2000, true, 2000), Rail::UsbC, 1000);  // accumulate
  uint32_t e_before = s.energy_mWh;
  TEST_ASSERT_TRUE(e_before > 0);
  session_update(s, mk(0, 0, false, 3000), Rail::UsbC, 1000);
  TEST_ASSERT_FALSE(s.active);
...
void test_reattach_resets_energy_and_start(void) {
  SessionStats s{};
  session_update(s, mk(9000, 2000, true, 1000), Rail::UsbC, 1000);
  session_update(s, mk(9000, 2000, true, 2000), Rail::UsbC, 1000);
  session_update(s, mk(0, 0, false, 3000), Rail::UsbC, 1000);
  session_update(s, mk(9000, 2000, true, 10000), Rail::UsbC, 1000);
  TEST_ASSERT_TRUE(s.active);
...
void test_energy_scales_with_dt(void) {
  ...
  SessionStats s{};
  session_update(s, mk(9000, 2000, true, 0), Rail::UsbC, 1000);  // attach edge, no dt
  // Drive an hour of 1s ticks.
  for (uint32_t t = 1; t <= 3600; ++t) {
    session_update(s, mk(9000, 2000, true, t * 1000), Rail::UsbC, 1000);
  }
...
void test_peak_current_is_session_max(void) {
  SessionStats s{};
  session_update(s, mk(5000, 300, true, 0), Rail::UsbC, 1000);
  session_update(s, mk(9000, 2000, true, 1000), Rail::UsbC, 1000);
  session_update(s, mk(9000, 1500, true, 2000), Rail::UsbC, 1000);
  TEST_ASSERT_EQUAL_UINT16(2000, s.peak_i_mA);
}

void test_peak_resets_on_reattach(void) {
  SessionStats s{};
  session_update(s, mk(9000, 2000, true, 0), Rail::UsbC, 1000);
  session_update(s, mk(0, 0, false, 1000), Rail::UsbC, 1000);
  session_update(s, mk(5000, 300, true, 2000), Rail::UsbC, 1000);
